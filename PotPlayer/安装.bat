...
:MENU
ECHO.&ECHO  1、设置优化（启用解码器及优化设置）
ECHO.&ECHO  2、文件关联（关联常见的音视频文件）
...
reg add "HKCU\Software\DAUM\PotPlayerMini64\Settings" /f /v "ScreenFitMode" /t REG_DWORD /d "0" >NUL 2>NUL
...
reg add "HKCU\Software\DAUM\PotPlayerMini64\Settings" /f /v "PlayScreenSize" /t REG_DWORD /d "2" >NUL 2>NUL
...
reg add "HKCU\Software\DAUM\PotPlayerMini64\Settings" /f /v "StartScreenSize" /t REG_DWORD /d "1" >NUL 2>NUL
...
reg add "HKCU\Software\DAUM\PotPlayerMini64\Settings" /f /v "IntH265_0" /t REG_DWORD /d "2" >NUL 2>NUL
...
reg add "HKCU\Software\DAUM\PotPlayerMini64\Settings" /f /v "RestoreDefault" /t REG_DWORD /d "0" >NUL 2>NUL
